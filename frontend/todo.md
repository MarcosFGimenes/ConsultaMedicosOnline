# Todo List

- [ ] Refazer as rotas de agendamento e agendamento imediato
  - Elas estão seguindo a documentação do Rapidoc, no entanto não é possivel fazer nenhum agendamento pois sempre retorna ```Cliente não possui essa especialidade cadastrada```

- [ ] Conectar telas de agendamento com o backend
  - Isso inclui no dashboard que aparece informações de agendamentos proximos

- [ ] Botão exportar meus dados e Excluir minha conta

- [ ] Conectar a função de cancelar plano

- [x] Logs de erros

- [ ] Conexão da tela de Meus Dados com o backend

